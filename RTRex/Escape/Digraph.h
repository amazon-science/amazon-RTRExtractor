...
struct DegreeComp
{
    ...
    bool operator () (VertexIdx u, VertexIdx v)
    {
        ...
        {
            printf("Something wrong in DegreeComp, negative vertices. u is %" PRId64 ", v is %" PRId64 "\n",u,v);
            exit(EXIT_FAILURE);
        }
...
CDAG degreeOrdered(CGraph *g)
{
    ...
    for (VertexIdx i=0; i < g->nVertices; ++i)   // Looping over all vertices in g
    {
        for (EdgeIdx j = g->offsets[i]; j < g->offsets[i+1]; ++j)   // Looping over neighbors of i in g
        {
            ...
            //printf("i=%" PRId64 " dest=%" PRId64 " degi=%" PRId64 " degdest=%" PRId64 "\n",i,dest,degi,degdest);
...
//         printf("Removed %" PRId64 ", min deg = %" PRId64 ", position = %" PRId64 "\n",to_remove,min_deg,deg_ptrs[min_deg]); //(for debug)
...
//             printf("Moving %" PRId64 " from %" PRId64 " to %" PRId64 "\n",nbr,vert_ptrs[nbr],deg_ptrs[degrees[nbr]]);
    ...
    for (VertexIdx i=0; i < g->nVertices; i++)
    {
        ...
        {
            printf("Error in degeneracy removal. Vertex %" PRId64 " not removed\n",i);
            exit(EXIT_FAILURE);
        }
    }
